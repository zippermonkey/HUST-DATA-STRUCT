...
// 多表操作
status SaveinMultiList(Lists *Ls, SqList *L); // 将线性表放入多表中
status InitMultiList(Lists **Ls);			  // 初始化多表
status ShowLists(Lists *Ls);				  //显示多表   如果执行的销毁操作...
...
int main(void)
{
	...
	while (op)
	{
		...
		printf("    	  15. LoadList       16. 初始化多表\n");
		printf("    	  17. 加入多表       18. 显示多表\n");
		printf("    	  0. Exit\n");
		printf("-------------------------------------------------\n");
		printf("请选择你的操作[0~18]:");
		scanf("%d", &op);
		...
		case 15:
			...
			break;
		case 16:
			sta = InitMultiList(&Ls);getchar();
			getchar();
			break;

		case 17:
		sta = SaveinMultiList(Ls,L);getchar();
			getchar();
			break;
		case 18:
		sta = ShowLists(Ls);getchar();
			getchar();
			break;
...
status InitMultiList(Lists **Ls)
{
	// 初始化多表
	(*Ls) = (Lists *)malloc(sizeof(Lists));
	if (*Ls == NULL)
	{
		// Ls动态内存分配失败
		return OVERFLOW;
	}
	(*Ls)->lists = (SqList **)malloc(sizeof(SqList *) * LIST_INIT_SIZE);
	if ((*Ls)->lists == NULL)
	{
		// L->elem动态内存分配失败
		return OVERFLOW;
	...
	(*Ls)->listsnum = 0;
	(*Ls)->listssize = LIST_INIT_SIZE;
	return OK;
}

status SaveinMultiList(Lists *Ls, SqList *L)
{
	// 将线性表放入多表中
	if (!Ls) // 多表未创建
		return INFEASTABLE;
	...
	if (Ls->listsnum >= Ls->listssize)
	{ // 空间不足 重新分配
		SqList **newbase = (SqList **)realloc(Ls->lists, (Ls->listssize + LISTINCREMENT) * sizeof(SqList *));
		if (!newbase)
			exit(OVERFLOW);
		Ls->lists = newbase;
		Ls->listssize += LISTINCREMENT;
	}

	Ls->lists[Ls->listsnum] = L; // 将线性表放入多表中
	Ls->listsnum++;
	return OK;
}
status ShowLists(Lists *Ls)
{
	//显示多表   如果执行的销毁操作...(暂未考虑)
